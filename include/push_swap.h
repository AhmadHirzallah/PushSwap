...
1. Modifying the Value (Content) of the First Element:
	If you only want to change the value stored at the first element (e.g., change "ahmad" to "ALI"), you only need two pointers: char **argv.
	This is because you're modifying the content at the memory location, not the pointer's address itself.
...
If you want to allocate new memory and change the pointer's address itself (e.g., pointing to a newly allocated memory), you need three pointers: char ***argv.
This is because you need to change the pointer's address in the calling function, which requires a pointer to the pointer (char ***argv).

void allocate_new(char ***argv)
{
    *argv = malloc(2 * sizeof(char *));  // Allocating new memory for the array of strings
    (*argv)[0] = strdup("ALI");          // Allocating and setting the first string
...
- But input is declared as: 	char **input; -> This means it expects a char **, not a char *.
*/
...
1. *stacks = ft_calloc(1, sizeof(t_stacks));
	- We are changing the value stored in the pointer, which is an address (*stacks).
	- We are not changing the address of the pointer itself.
	- The pointer still lives in the same place in memory; we're just updating where it points.
...
/*
** read_update_indices:
** Updates node indices in the circular list and returns the length of the stack :)
*/
